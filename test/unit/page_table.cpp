...
class Generic_pointer
{
    ...
    public:
        ...
        this_t operator+(size_t offset) const
        {
            return { addr + offset*sizeof(ENTRY) };
        }
    ...
    public:
        ...
        class iterator {
            ...
            public:
                ...
                Fake_memory operator*() const
                {
                    return Fake_memory {{it_, memory_->memory_.cend()}};
                }

                bool operator==(iterator const &rhs)
                {
                    return memory_ == rhs.memory_ and it_ == rhs.it_;
                }
                ...
                iterator(Fake_memory const *memory)
                    : memory_ (memory), it_ (memory->memory_.cbegin())
                {}
        };
        ...
        Fake_memory(memory_list const &memory = {})
            : memory_ {memory}
        {}
...
class Fake_page_alloc
{
    ...
    public:
        ...
        freed_memory_list const &get_freed_pages() const
        {
            return freed_;
        }
...
class Fake_deferred_cleanup
{
        ...
        Fake_deferred_cleanup(bool tlb_flush, pointer_vector const &lazy_free)
            : tlb_flush_ {tlb_flush},
              lazy_free_pages_ {lazy_free}
        {}
    ...
    public:
        ...
        void merge(Fake_deferred_cleanup &other)
        {
            ...
            lazy_free_pages_.insert (lazy_free_pages_.end(),
                                     other.lazy_free_pages_.cbegin(),
        ...
        static Fake_deferred_cleanup tlb_flush(bool tlb_flush)
        {
            return {tlb_flush, {}};
        }
...
TEST_CASE("Can split 1GB pages into 2MB pages", "[page_table]")
{
    ...
    Fake_deferred_cleanup cleanup;
    auto const split_res {hpt.walk_down_and_split (cleanup, 0, 1)};
    CHECK(cleanup.need_tlb_flush());
...
TEST_CASE("Update creates new mappings in empty page table", "[page_table]")
{
    ...
    auto create_and_check = [] (Fake_hpt &hpt, Fake_hpt::virt_t vaddr, Fake_hpt::phys_t paddr,
                                ...
                                auto const cleanup {hpt.update ({vaddr, paddr,
                                                                 Fake_attr::PTE_P | Fake_attr::PTE_W, order})};
                                CHECK_FALSE(cleanup.need_tlb_flush());
...
TEST_CASE("Updates are atomic", "[page_table]")
{
    ...
    SECTION("Superpage splitting is atomic") {
        ...
        for (auto it {hpt.memory().now()}; it != before_mapping; ++it) {
            ...
            REQUIRE((cur_map == old_gigabyte_mapping ||
                     cur_map == old_twomb_mapping ||
                     cur_map == old_page_mapping ||
...
TEST_CASE("Replacing read-only pages works", "[page_table]")
{
    ...
    SECTION("Non-existing mapping is created") {
        verify_mapping (hpt.replace_readonly_page (cleanup, 0x4000, 0x3000, Fake_attr::PTE_P | Fake_attr::PTE_W),
                        0x4000, 0x3000);
    }

    SECTION("Existing read-only page is replaced") {
        verify_mapping (hpt.replace_readonly_page (cleanup, 0, 0x3000, Fake_attr::PTE_P | Fake_attr::PTE_W),
                        0, 0x3000);
    }
