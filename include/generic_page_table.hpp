...
public:
    ...
    public:
        ...
        WARN_UNUSED_RESULT Mapping move_by(virt_t offset) const
        {
            ...
            return {vaddr + offset, paddr, attr,
                    min<ord_t>(order, static_cast<ord_t>(::max_order(0, offset)))};
        }
    };
...
private:
    ...
    retry:
        ...
        return walk_down_and_split(cleanup, vaddr, to_level, page_alloc_.phys_to_pointer(phys), cur_level - 1,
                                   create);
    }
    ...
    NOINLINE void cleanup(DEFERRED_CLEANUP& cleanup_state, pte_t pte, level_t cur_level)
    {
        ...
        if (is_leaf(cur_level, pte)) {
            ...
            cleanup_table(cleanup_state, page_alloc_.phys_to_pointer(pte & ~ATTR::mask), cur_level);
        }
    }
                               ...
                               Mapping const& map)
    {
        ...
        for (size_t i = 0; i < (static_cast<size_t>(1) << updated_order); i++) {
            ...
            retry:
                ...
                Mapping const sub_map{map.vaddr + addr_offset, map.paddr + addr_offset, map.attr,
                                      entry_order};

                fill_entries(cleanup_state, page_alloc_.phys_to_pointer(old_pte & ~ATTR::mask), cur_level - 1,
                             sub_map);
            }
        }
...
public:
    ...
    WARN_UNUSED_RESULT phys_t replace_readonly_page(DEFERRED_CLEANUP& cleanup, virt_t vaddr, phys_t paddr,
                                                    pte_t attr)
    {
    ...
    Generic_page_table(level_t max_levels, level_t leaf_levels, pte_pointer_t root, MEMORY const& memory = {})
        : memory_{memory}, max_levels_{max_levels}, leaf_levels_{leaf_levels}, root_{root}
    {
