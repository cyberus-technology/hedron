...
#define CPULOCAL_REMOTE_ACCESSOR(prefix, field) static auto remote_ref_ ## field(unsigned cpu) \
        -> CPULOCAL_TYPE(prefix, field) &                                                      \
    { return Cpulocal::get_remote(cpu).CPULOCAL_FIELD_NAME(prefix, field); }                   \
                                                                                               \
    template <typename T = void>                                                               \
    static auto remote_load_ ## field(unsigned cpu) -> CPULOCAL_TYPE(prefix, field)            \
