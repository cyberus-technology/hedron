...
Pci::Pci(unsigned r, unsigned l)
    : Forward_list<Pci>(list), reg_base(hwdev_addr -= PAGE_SIZE), rid(static_cast<uint16>(r)),
      lev(static_cast<uint16>(l))
{
...
{
    for_each(Forward_list_range{list}, [d](Pci& pci) {
        if (not pci.dmar) {
            pci.dmar = d;
        }
    });
...
bool Pci::claim_dev(Dmar* d, unsigned r)
{
    ...
    auto end = find_if(++devices.begin(), devices.end(), [l](Pci const& dev) { return dev.lev <= l; });
