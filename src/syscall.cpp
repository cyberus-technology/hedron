...
void Ec::sys_create_ec()
{
    ...
    Ec *ec = new Ec (Pd::current, r->sel(), pd, r->flags() & 1 ? static_cast<void (*)()>(send_msg<ret_user_iret>) : nullptr, r->cpu(), r->evt(), r->utcb(), r->esp(), r->vlapic_page());
...
void Ec::sys_pt_ctrl()
{
    ...
    if (EXPECT_FALSE (sanitize_cap(cap, Kobject::PT, 1UL << 0))) {
