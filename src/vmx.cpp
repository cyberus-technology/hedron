...
void Vmcs::init()
{
    ...
    if (not has_ept() or not has_urg() or not has_guest_pat() or not has_msr_bmp() or
    ...
    trace(TRACE_VMX, "VMCS:%#010lx REV:%#x EPT:%d URG:%d VNMI:%d VPID:%d", Buddy::ptr_to_phys(root),
          basic().revision, has_ept(), has_urg(), has_vnmi(), has_vpid());
}
