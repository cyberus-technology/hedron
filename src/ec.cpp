...
Ec::Ec (Pd *own, mword sel, Pd *p, void (*f)(), unsigned c, unsigned e, mword u, mword s, bool vcpu, bool use_apic_access_page) : Kobject (EC, static_cast<Space_obj *>(own), sel, 0xd, free, pre_free), cont (f), pd (p), cpu (static_cast<uint16>(c)), glb (!!f), evt (e), user_utcb (u), xcpu_sm (nullptr)
{
    ...
    if (not vcpu) {
        ...
        pd->Space_mem::insert (u, 0, Hpt_new::PTE_NX | Hpt_new::PTE_U | Hpt_new::PTE_W | Hpt_new::PTE_P, Buddy::ptr_to_phys (utcb));
        ...
        if (Hip::feature() & Hip::FEAT_VMX) {
            ...
            if (u) {
                ...
                pd->Space_mem::insert (u, 0, Hpt_new::PTE_NX | Hpt_new::PTE_U | Hpt_new::PTE_W | Hpt_new::PTE_P, vlapic_page_p);
...
void Ec::root_invoke()
{
    Eh *e = static_cast<Eh *>(Hpt_new::remap (Hip::root_addr, false));
    ...
    ELF_PHDR *p = static_cast<ELF_PHDR *>(Hpt_new::remap (Hip::root_addr + e->ph_offset, false));
