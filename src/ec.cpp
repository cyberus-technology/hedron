...
Ec::Ec (Pd *own, mword sel, Pd *p, void (*f)(), unsigned c, unsigned e, mword u, mword s, mword v) : Kobject (EC, static_cast<Space_obj *>(own), sel, 0xd), cont (f), pd (p), prev (nullptr), next (nullptr), cpu (static_cast<uint16>(c)), glb (!!f), evt (e), timeout (this)
{
    ...
    if (u) {
        ...
        if (Hip::feature() & Hip::FEAT_VMX) {
            ...
            if (v) {
                /* allocator+register the virtual LAPIC page and map it into user space */
                ...
                pd->Space_mem::insert (v, 0, Hpt::HPT_U | Hpt::HPT_W | Hpt::HPT_P, vlapic_page);
            }

            regs.vmcs->clear();
            cont = send_msg<ret_user_vmresume>;

            trace (TRACE_SYSCALL, "EC:%p created (PD:%p VMCS:%p VLAPIC:%lx)", this, p, regs.vmcs, v);
